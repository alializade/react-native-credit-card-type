...
var visaCards = creditCardType.detectCreditCard('4111');
console.log(visaCards[0].type);  // 'visa'

var ambiguousCards = creditCardType.detectCreditCard('6');
...
`detectCreditCard` will return an array of objects, each with the following data:
...
```javascript
creditCardType.detectCreditCard(cardNumber).filter(function(card) {
